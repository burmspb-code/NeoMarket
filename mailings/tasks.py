...
def send_single_mailing_task(mailing_id):
    ...
    try:
        ...
        mailing = (
            MailingManagement.objects.select_for_update()
    ...
    if mailing.status == "completed":
        print(
            f"[Celery Защита] Перехват дубликата! Рассылка {mailing_id} уже была отправлена."
        )
...
def check_and_run_mailings():
    ...
    expired_mailings = MailingManagement.objects.filter(
        end_time__lt=now, status__in=["created", "launched"]
    )
